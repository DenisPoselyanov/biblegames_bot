...
async function postRow(input: PostRowInput): Promise<WalletPostResult> {
  ...
  if (inserted.rows[0]) {
    return { entry: rowToEntry(inserted.rows[0]), replayed: false };
  }
  ...
  if (existing.rows[0]) {
    return { entry: rowToEntry(existing.rows[0]), replayed: true };
  }
...
export function createSqlWalletLedger(): WalletLedger {
  return {
    ...
    async post(input) {
      ...
      return postRow({ ...input, id: randomUUID() });
    },
