...
### WS2 — Content lifecycle model (questions + lessons)
- Resolve open decision #1 above via ADR before implementation.
- `ContentRevision`/draft records with field-level diff support (what WS8's review editor renders) for both questions (extending the existing `QuestionRevisionRecord`) and lessons/lesson-blocks (new — Phase 3 shipped the tables but no revision/diff layer over them).
- `correctIndex` (and lesson-block required fields) never fall back to a default on invalid input — validation rejects, never silently repairs (§22).
- DB-level write restriction so AI-originated writes cannot reach the published store directly (§3.2/§22) — a permission/role check, not a convention.
