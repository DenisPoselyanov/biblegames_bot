...
### WS10 — Hardening, rollout, DoD sign-off — **close-out done (2026-09-21)**, see below
...
- **§23 close-out (2026-09-21, `phase-3/ws10-rollout-closeout`)**: full suite run at 453/453 (`npx vitest run`, zero flake this run — the 5 shop/wallet-under-pglite timeouts documented earlier are confirmed-flaky-under-load, reproduced clean in isolation). Closed 2 of the unit-test-category gaps that had zero coverage despite being explicitly listed: "progress/number event deduplication" (`src/lib/eventDedup.test.ts`, against a fake `localStorage` since this suite's `environment: 'node'` has no browser globals) and "reduced motion" (`MotionProvider`'s particles/haptic/sound gating was inline in a `useMemo` with no way to unit-test it without a React rendering harness this repo doesn't have — extracted to a pure `deriveMotionCapabilities()` in `src/lib/motionCapabilities.ts`, wired back in, tested directly). **Not closed, not hidden**: "AnswerOption states" (still needs a component-rendering harness this repo has never had); the full E2E/manual matrix and the visual-regression baseline (§23) — **no visual regression tooling exists in this repo at all**, this was never built in any earlier phase, not a Phase 3 regression, and picking/building one is a fresh scope decision for later, not something to invent unprompted here.
- **§20/§26 close-out (2026-09-21)**: product-owner decision — in place of a live staged-percentage rollout (needs production metrics/ops access this agent doesn't have), all 8 rollout flags (`learningShellV2`, `today_dashboard`, `learning_plans`, `lesson_experience_v2`, `practiceSessionV2`, `progress_dashboard_v2`, `profileSettingsV2`, `lightThemeDefault`) were flipped to `true` in `src/lib/flags.ts`'s `FLAG_DEFAULTS`, treating that as the phase's full-rollout step; the legacy `Layout`/`Home`/`ThemeDetail`/`Profile` tree is kept, unreachable, as the rollback target rather than deleted (§20's "remove old shell after window" stays deliberately deferred to a real retention window against production traffic). Also closed the deferred WS10 §17 follow-up (`task_7ea52d9a`): the same gold-on-ivory contrast bug the accessibility pass fixed in the new semantic-token layer also existed as a raw `color: var(--gold-light)` in ~62 declarations across 21 pre-WS3 files (Millionaire, Survival, Kahoot, Quiz, Admin, Social, etc.) — all switched to `--accent-spiritual-text`, which is byte-identical to the old value for every theme except `light` (where it's the already-fixed ~5:1+ darkened variant), so this is a pure contrast fix with no visual change elsewhere. Verified: `tsc -b` + server `tsc` clean, `eslint` clean (same 2 pre-existing `react-refresh/only-export-components` hits on `Toast.tsx`/`MotionProvider.tsx` already documented), full suite green, and live dev-server QA with **no `.env.local` overrides** (i.e. exercising the shipped code defaults exactly as a production build would): fresh load → `Today` inside the v2 shell with `light` theme applied; `/play/millionaire` (always-reachable regardless of these flags) renders the contrast fix correctly; rollback path exercised (`VITE_FLAG_LEARNINGSHELLV2=false` + `VITE_FLAG_LIGHTTHEMEDEFAULT=false`, dev server restarted since Vite bakes `VITE_*` at start) → legacy 4-tab shell renders correctly, confirming DoD §25.1 "rollback-tested". Full mechanics, the flag table, and the §26 rollback-requirement-by-requirement check are in the new [`docs/ROLLOUT_PHASE_3.md`](../../docs/ROLLOUT_PHASE_3.md).
- **§27 Phase 4 handoff doc — done**: [`docs/phases/PHASE_3_TO_4_HANDOFF.md`](../../docs/phases/PHASE_3_TO_4_HANDOFF.md), mapping each of §27's received-items to the actual code, plus an explicit "open items" section (no published content, no visual-regression suite, no on-device Telegram testing, product-owner sign-off pending).
- **DoD tie-in**: closes §25 items 14–18. Item 2 is mechanically done but produces empty states until Phase 4 publishes content (not a Phase 3 defect). Item 19 (product-owner sign-off) cannot be closed by an agent — see the handoff doc.
