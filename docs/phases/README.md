...
4.5. [`PHASE_3_5_DESIGN_V2_VISUAL_MIGRATION.md`](./PHASE_3_5_DESIGN_V2_VISUAL_MIGRATION.md) — **owner-approved interstitial phase, не з оригінальної нумерації master specification.** Переносить увесь застосунок з палітри `Світло` на затверджений напрям `proto/design-v2` (Hallow-style): токени, типографіка, компоненти — без зміни IA, reward-логіки чи серверних контрактів.
5. [`PHASE_4_CONTENT_AI_AND_CONTENT_STUDIO.md`](./PHASE_4_CONTENT_AI_AND_CONTENT_STUDIO.md) — content lifecycle, unified authoring, quality audit, reviewed AI pipeline, media proposals та protected Content Studio (візуальний/IA референс — прототип `/proto/studio`, той самий `proto/design-v2`).
