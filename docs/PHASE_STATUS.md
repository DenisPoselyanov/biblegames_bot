...
| 3 | Learning Product, MVP, Rebrand & Motion | **in progress (started 2026-09-14).** Phase 1–2 unblocked (code-complete in `main`); execution plan: `.claude/plans/phase-3-learning-shell-rebrand.md` (WS1–WS10). **WS1** (`phase-3/ws1-learning-domain`, ADR-017): new `learning` domain — typed `learning_plans` / `learning_modules` (self-referencing `parentModuleId`) / `learning_objectives` / `lessons` / `lesson_blocks` (migration `0007_learning_domain`), depth-agnostic mapping from `data/topics-db/*.json` reusing existing `themeId`/`topicNodeId` vocabulary, `scripts/migrate/map-learning-content.ts` (idempotent, `--dry`, requires `DATABASE_URL`). All rows land `legacy_unreviewed` (ADR-004 rule) — Phase 4 owns authoring/review, WS2 owns the read API and publication policy. Verified: 115/115 tests green incl. new `learning` repository contract (in-memory + pglite) and `schemaParity` allowlist; mapping logic smoke-tested against all 17 real topic files (17 plans/108 modules/460 objectives/460 lessons, 0 duplicate ids). `drizzle-kit generate` did not run in this environment (node_modules corruption, tracked separately) — migration hand-authored per the `0000`–`0006` format and proven correct via pglite instead; `meta/0007_snapshot.json` still needs regenerating in a clean environment before the next migration on these tables. **Remaining Phase 3 work**: WS2–WS10 per the plan doc (not started). Legacy UI (`Home.tsx`/`Lesson.tsx`/`StudyHub.tsx`/`ReviewQueue.tsx`/`ProgressDashboard.tsx`) is pre-Phase-3 and still the live surface until WS5+ ships. | [Phase 3](./phases/PHASE_3_LEARNING_PRODUCT_REBRAND_AND_MOTION.md) |
