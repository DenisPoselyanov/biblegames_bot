...
| 1 | Production Safety & Engineering Foundation | **in progress** — WS1 landed (env validation, hardened Telegram initData verifier, typed principal, fail-closed HTTP + Socket auth, `createApp()` split, request-id + error envelope, Vitest + CI gates); WS2 landed (config-sourced RBAC roles/permissions, `requireRole`/`requirePermission` policy middleware, isolated + audited `/api/admin/questions`, append-only audit log, self-scoped `/api/v1/me/*`); WS3 landed (immutable wallet ledger + idempotency, `POST /api/v1/progression/completions` server-authoritative reward command with stable `eventId`, preference/progression write split, one-time bounded legacy migration, atomic JSON adapter — all behind flags default OFF); WS4 client cutover + flag flip + legacy removal + DoD sign-off still open | [Phase 1](./phases/PHASE_1_PRODUCTION_SAFETY_AND_ENGINEERING_FOUNDATION.md) |
