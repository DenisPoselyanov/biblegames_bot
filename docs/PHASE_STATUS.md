...
| 2 | Core Architecture & Authoritative Data Platform | ready — Phase 1 handoff complete; modular monolith, Drizzle/PostgreSQL schema, revisions, jobs, object storage і read models. Consolidates the WS4-part-2 minimal services (in-memory rate limit / metrics → shared backend; config-sourced RBAC → persisted role store) | [Phase 2](./phases/PHASE_2_CORE_ARCHITECTURE_AND_DATA_PLATFORM.md) |
